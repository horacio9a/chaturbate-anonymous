# Chaturbate Remote FFPLAY/FFMPEG Anonymous Freechat Recorder v.1.0.2 by horacio9a for Python 2.7.13
...
if "HTTP 404" not in dec:
 ...
 if len(pwd) < 3:
   ...
   if len(hlsurl1) > 1:
      ...
      command = ('ffmpeg -hide_banner -loglevel panic -i {} -c:v copy -c:a aac -b:a 160k -timeout 600 {}'.format(hlsurl,pf))
      os.system(command)
      print(colored("\n => END <= ", 'yellow','on_blue'))
      sys.exit()
   else:
